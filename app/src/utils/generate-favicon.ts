...
const svg = (color: string, addDirectusLogo: boolean): SVGSVGElement => {
	...
	// Setting via CSSOM (not string interpolation into markup) means the browser
	// itself validates the value — invalid/malicious input is dropped, not parsed as markup.
...
export const generateFavicon = (color: string, addDirectusLogo = true) => {
	...
	const svgEl = svg(color, addDirectusLogo);
	const iconSerialized = new XMLSerializer().serializeToString(svgEl);
